...
class RteApiService
{
    ...
    public function authenticate()
    {
        ...
        $cacheToken = config('services.rte.cache_token', false);

        // Check if token is cached
        if ($cacheToken && cache()->has('rte_api_token')) {
    ...
    public function fetchGenerationPerUnit(?Carbon $start = null, ?Carbon $end = null): ?array
    {
        ...
        // Check for successful response
        if ($response->failed()) {
            throw new \Exception('Failed to fetch data from RTE API ('.$url.'): Code '.$response->status().' - '.$response->body());
        }
