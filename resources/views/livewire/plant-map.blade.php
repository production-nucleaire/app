            ...
            <div class="mx-5 md:mx-6 my-4 bg-surface border border-line rounded-[12px] px-4 pt-3.5 pb-2"
                ...
                <div class="flex items-center gap-4 pb-2.5 flex-wrap">
                    ...
                    <div class="flex gap-3 font-mono text-[11px] text-muted flex-wrap">
                        @foreach ($reactors as $i => $reactor)
                            <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full" style="background:{{ $tints[$i % count($tints)] }}"></span>T{{ $reactor->reactor_index }}</span>
                        @endforeach
                    </div>
...
<script>
    ...
    if (boot.reactor && reactorEl && window.createReactorChart) {
        window.reactorChart = createReactorChart(reactorEl, boot.reactor.series, { slug: boot.reactor.slug, tints: boot.reactor.tints });
    }
</script>
@endscript
