<div class="flex items-center justify-between border-t border-t-slate-200 pt-2 mt-auto">
    @if ($previousPlant)
        <a href="{{ route('plant', ['slug' => $previousPlant->slug]) }}" class="h-8 flex items-center justify-center gap-1 bg-white hover:bg-slate-200 rounded text-xs font-medium text-slate-600 hover:text-slate-800 uppercase px-3 mr-auto" wire:click.prevent="$set('selectedPlantId', {{ $previousPlant->id }})">
    ...
    @endif
    @if ($nextPlant)
        <a href="{{ route('plant', ['slug' => $nextPlant->slug]) }}" class="h-8 flex items-center justify-center gap-1 bg-white hover:bg-slate-200 rounded text-xs font-medium text-slate-600 hover:text-slate-800 uppercase px-3 ml-auto" wire:click.prevent="$set('selectedPlantId', {{ $nextPlant->id }})">
            {{ $nextPlant->name }}
