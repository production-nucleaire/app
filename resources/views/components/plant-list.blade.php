<div x-data="{ mode: $persist('grid') }" class="flex flex-col gap-4">
    <div class="flex items-center justify-between gap-2">
        <div class="flex items-center gap-2">
            <button class="cursor-pointer" type="button" x-on:click="mode = 'grid'">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="w-4 h-4" x-bind:class="mode == 'grid' ? 'fill-slate-800' : 'fill-slate-400'"><!--!Font Awesome Pro v7.0.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2025 Fonticons, Inc.--><path d="M288 96L96 96L96 288L288 288L288 96zM288 352L96 352L96 544L288 544L288 352zM352 96L352 288L544 288L544 96L352 96zM544 352L352 352L352 544L544 544L544 352z"/></svg>
            </button>
            <button class="cursor-pointer" type="button" x-on:click="mode = 'list'">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" class="w-4 h-4" x-bind:class="mode == 'list' ? 'fill-slate-800' : 'fill-slate-400'"><!--!Font Awesome Pro v7.0.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2025 Fonticons, Inc.--><path d="M176 112L80 112L80 208L176 208L176 112zM256 128L224 128L224 192L576 192L576 128L256 128zM256 288L224 288L224 352L576 352L576 288L256 288zM256 448L224 448L224 512L576 512L576 448L256 448zM80 272L80 368L176 368L176 272L80 272zM176 432L80 432L80 528L176 528L176 432z"/></svg>
            </button>
        </div>
    ...
    <div class="grid gap-2" x-bind:class="mode == 'list' ? 'grid-cols-1' : 'grid-cols-2 md:grid-cols-3'">
        ...
        @foreach ($plants as $plant)
            <a
                class="flex items-center justify-between bg-slate-50 hover:bg-slate-200 border border-slate-50 rounded p-2"
            ...
            >
                <div>
                    <h3 class="text-sm font-semibold text-slate-800">{{ $plant['name'] }}</h3>
                    <div class="text-[.65rem] font-semibold text-slate-600">{{ sprintf('%d tranche%s sur %d', $plant['active_reactors'], $plant['active_reactors'] === 1 ? '' : 's', $plant['total_reactors']) }}</div>
