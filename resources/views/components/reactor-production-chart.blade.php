...
>
    <div class="flex items-center justify-between px-2">
        @if ($previousDay)
            <a href="{{ route('plant', ['slug' => $reactor->plant_id, 'reactor' => $reactor->id, 'day' => $previousDay->format('Y-m-d')]) }}" x-on:click.prevent="$wire.set('day', '{{ $previousDay->format('Y-m-d') }}')" class="w-10 h-10 flex items-center justify-center dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-900 rounded text-slate-600 hover:text-slate-800">
